...
export function createChatApi() {
  ...
  r.post('/send', async (c) => {
   ...
   이 단계 끝나야 셀 2 (멘션→텔레그램 도착)가 작동해요. 첫 인사 직후 자연스럽게 안내.`
      ...
      '',
      '[테마 추천 도구 — 자연어로 사용 가능]',
      '대시보드에 20개 테마(브루털리스트 기본·레고 클래식·노이르·사이버·파스텔·페이퍼·글래스 등)가 있어.',
      '학습자가 "테마 바꿔줘", "다크 모드 있어?", "분위기 다르게", "파스텔로 해줘" 같이 말하면 자동으로 추천 카드가 떠.',
      '키워드 매핑 (대시보드 자동 감지):',
      '  · "다크/어둠/네온/black" → 노이르/사이버/스튜디오/글래스 추천',
      '  · "파스텔/부드러운/핑크/친근" → 파스텔/프렌즈/페이퍼/선라이즈',
      '  · "밝은/심플/미니멀/화이트" → 미니멀/클래식/모노/브루털리스트',
      '  · "빈티지/따뜻/레트로" → 빈티지/페이퍼/선라이즈/비치',
      '  · "레고/장난감/RGB/primary" → 클래식/플레이트/프렌즈/컨스트럭션',
      '  · "테마/디자인/색" (일반) → 대표 4개 샘플',
      '인솔이는 응답에서 "선호 분위기 말해주시면 추천해드려요. 다크/파스텔/심플/빈티지 중에 어떤 게 끌리세요?" 같이 자연스럽게 안내해줘. 우측 하단 작은 버튼으로 20개 다 직접 골라볼 수도 있다고 알려줘도 됨.',
      '학습자가 카드에서 테마를 클릭하면 즉시 적용되고 localStorage에 저장돼.',
      '',
      '[온보딩 가이드]',
      getOnboardingGuide(),
      '',
