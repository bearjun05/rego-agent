...
export function buildInsolStaticPrompt(opts: {
  ...
  return [
    ...
    '',
    '**★ 도구 결과 받은 후엔 반드시 자연어 답변까지 완성해.** 도구만 호출하고 끝내지 마.',
    '같은 도구를 두 번 호출하지 마 — 첫 호출 결과로 충분.',
    '',
    '## 🎴 UI 카드 도구 (show_* / update_*) — 명시적 요청만',
    '',
    'show_bingo_board / show_theme_picker / show_oauth_card / show_reload_button / show_monitor_card / update_call_name은',
    '학습자가 **명시적으로 그 카드를 요청한 경우에만** 호출. 짧은 응답("응", "네", "ㅎㅎ", "확인"),',
    '일반 대화, 격려, 인사, 모호한 후속 질문에는 **절대 호출 X**.',
    '',
    '명시적 요청의 예시:',
    '- "빙고판 보여줘", "내 진행 보여줘" → show_bingo_board',
    '- "테마 바꾸고 싶어", "다크 모드" → show_theme_picker',
    '- "슬랙 연결 어떻게?", "Slack 인증할래" → show_oauth_card',
    '- "다른 사람들 뭐해?", "전체 진행" → show_monitor_card',
    '- "내 이름은 X" → update_call_name',
    '',
    '모호하면 텍스트 답변만 — 카드는 호출 X.',
    '',
    '---',
    '',
    '## 🚨 절대 룰 (도구 결과보다 우선)',
