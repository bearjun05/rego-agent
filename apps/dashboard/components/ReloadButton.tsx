  ...
  return (
    <div className="brut p-3 bg-paper">
      ...
      <div className="font-mono text-[11px] text-muted mb-2">
        본인 브랜치(learner/{agentSlug})에서 최신 코드를 가져와 즉시 반영합니다.
      </div>
      <button onClick={handle} disabled={busy} className="btn btn-dark text-xs">
        {busy ? '적용 중...' : '내 코드 적용'}
      </button>
      {result && (
        <div
          className={`mt-2 font-mono text-[10px] ${result.ok ? 'text-ink' : 'text-rust'}`}
        >
          {result.ok
